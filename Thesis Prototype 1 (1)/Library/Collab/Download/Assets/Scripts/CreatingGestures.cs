...
public class CreatingGestures : MonoBehaviour
{
    ...
    public float[] lightningHand;
    public float[] airHand;
    public float[] wardHand;
    public float[] waterHand;

    string spell;

    bool fireStart, lightningStart, airStart, wardStart, waterStart;
    ...
    void Update()
    {
        ...
        if (this.gr != null)
        {
            ...
            if (active_controller == null)
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    if(handAction.skeletonAction.fingerCurls[i] > .8)
                    {
                        spellSub = spell.ToCharArray();
                        spellSub[i] = 'a';
                    }
                    else if(handAction.skeletonAction.fingerCurls[i] < .3)
                    {
                        spellSub = spell.ToCharArray();
                        spellSub[i] = 'b';
                    }
                ...
                if ((handAction.skeletonAction.fingerCurls[0] > fireHand[0] && handAction.skeletonAction.fingerCurls[1] < fireHand[1] && handAction.skeletonAction.fingerCurls[2] < fireHand[2] && handAction.skeletonAction.fingerCurls[3] 
                    > fireHand[3] && handAction.skeletonAction.fingerCurls[4] > fireHand[4]) || (Input.GetKey(KeyCode.A) && Input.GetKey(KeyCode.B)))
                {
